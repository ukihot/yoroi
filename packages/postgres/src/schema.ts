...
export const queueEntry = pgTable(
	"queue_entry",
	{
		...
		repoId: text("repo_id").notNull(),
		prNumber: integer("pr_number").notNull(),
		lane: text("lane").notNull().$type<"default" | "hotfix" | "high_risk" | "mega">().default(
			"default",
		),
...
export const blockedEntry = pgTable(
	"blocked_entry",
	{
		...
		repoId: text("repo_id").notNull(),
		prNumber: integer("pr_number").notNull(),
		responsibility: text("responsibility").notNull().$type<
			| "your_action"
			| "other_reviewer"
			| "ci"
			| "queue"
...
export const fleetHealthSnapshot = pgTable(
	...
	{
		installationId: bigint("installation_id", { mode: "number" }).notNull(),
		component: text("component").notNull().$type<
			"control" | "merger" | "console" | "github_api" | "evidence_export"
		>(),
...
export const approvalCarryForward = pgTable(
	"approval_carry_forward",
	{
		...
		carriedForwardAt: timestamp("carried_forward_at", { withTimezone: true }).notNull()
			.defaultNow(),
	},
