...
Deno.test(
	'compilePolicy: repoレベルのoverrideはexplicitにorgをkey単位で置き換える (P-08: 暗黙のlast-match禁止)',
	async () => {
		const org = validDoc();
...
Deno.test(
	'compilePolicy: repoのscopes配列指定はorgのscopesを丸ごと置き換える（要素単位merge禁止）',
	async () => {
		const org = validDoc();
		const repoOverride = {
			scopes: [
...
Deno.test(
	'compilePolicy: 空の必須check集合はEMPTY_REQUIRED_CHECK_SETで安全側errorになる (FR-012)',
	async () => {
		const doc = validDoc({
			scopes: [
				{
