...
export const handleRecheck: CommandHandler = async (ctx, _args, ports) => {
	const cooldownKey = `recheck:${ctx.repo.repositoryId}:${ctx.pullRequestNumber}:${ctx.observedHeadSha}`;
	...
	if (freshHeadSha !== ctx.observedHeadSha) {
		return {
			kind: 'pending',
			markdown: `headが更新されています（新SHA: ${freshHeadSha}）。新しいheadに対する判定を待っています。`
		};
	}
...
export const handleQueue: CommandHandler = async (ctx, _args, ports) => {
	...
	const etaRange: readonly [Date, Date] | null =
		snapshot.etaFrom && snapshot.etaTo ? [snapshot.etaFrom, snapshot.etaTo] : null;
	return {
		kind: 'ok',
		markdown:
			`queue位置: ${snapshot.position ?? '不明'}（lane: ${snapshot.lane}）\n` +
...
export const handleFlakySubcommand: CommandHandler = async (ctx, args, ports) => {
	...
	if (subcommand === 'report') {
		...
		return {
			kind: 'ok',
			markdown:
				`flaky候補として記録しました（confidence: ${result.confidence}, ` +
	...
	if (subcommand === 'quarantine-request') {
		...
		return {
			kind: 'ok',
			markdown: `quarantine提案を作成しました（proposal: ${proposal.proposalId}）。承認者の承認待ちです。`
		};
	}
