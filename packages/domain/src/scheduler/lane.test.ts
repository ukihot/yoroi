...
Deno.test("rebuildAfterEjection: ejectedIndexより後のlaneはcandidateSha/statusがリセットされる", () => {
	const lanes = [lane("A", 1), lane("B", 2), lane("C", 3)];
	const rebuilt = rebuildAfterEjection(lanes, 1);
...
Deno.test("rebuildAfterEjection: ejectedIndex位置のheadが後続laneのcumulativeHeadsから除かれる", () => {
	const lanes = [lane("A", 1), lane("B", 2), lane("C", 3)];
	const rebuilt = rebuildAfterEjection(lanes, 1);
